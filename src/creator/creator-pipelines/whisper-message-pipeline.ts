...
import { formatEntryContent, type SessionStore } from "../../data/session-store.js";
...
export class WhisperPipeline {
	...
	async runWhisper(
		...
		if (targetEntry === undefined || targetSessionId === null || targetSocket === null) {
		...
		try {
			this.entryId = this.deps.sessionStore.appendCustomMessageEntry(
				...
				formatEntryContent(senderName, message.params.content),
				true,
				details,
			);
		} catch (error) {
