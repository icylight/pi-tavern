...
export function wireAgentLifecycle(pi: ExtensionAPI, ctrl: TavernController): void {
	...
	pi.on("agent_start", () => {
		...
		if (state.type === "character") {
			...
			// agent_start 续命——取消上一次 agent_end 布防的 5s 显示复位
			// watchdog（continue 段边界：agent_end → 毫秒级 continue → agent_start			// 不清除则定时器在段内 LLM 调用 >5s 时误灭灯，长 run 收尾段最易触发）。
			state.runtime.clearStreamingResetWatchdog();
			// agent_start 布防 run wedged watchdog（W 内无 agent_settled →
	...
	pi.on("agent_end", () => {
		const state = ctrl.getState();
		if (state.type === "character") {
			// /布防流式复位 watchdog。若 agent_settled 永不
	...
	pi.on("agent_settled", () => {
		const state = ctrl.getState();
		if (state.type === "character") {
			// 统一 settle 路径（含 run watchdog 清除与 wedged 幂等合并）。
