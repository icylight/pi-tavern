...
/**
 * 响应 result 形状判别（fail-close）：JSON-RPC 响应无 method，库
 ...
 * （方案 B：feed 前拦截 + id→method 关联表删除，语义等价）。
 */
const RESPONSE_RESULT_MATCHERS: Record<string, (result: unknown) => boolean> = {
	...
	// whisper 三态（published / stale / round_limit_reached，与 speak 同构
	// ——published 字面量判别；复用语义核对表第 1 轮即补齐，防漏注册）。
	// 收紧：不仅判别字段，还验证 runtime 随后会读取的必需
...
/**
 * 解析时形状校验（方案 B：替代 ResponseCorrelator feed 前拦截）。
