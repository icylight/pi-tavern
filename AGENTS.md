...
- 游标按 Session 隔离：`cursors/<groupId>/<sessionId>.json`；旧群聊级单文件不读不写不删；join 消费路径预置游标 = 进入时刻水位（CAS 写，零 wire 变化）；进入前历史不自动注入，经 `tavern_history` 工具 AI 主动分页拉取（欢迎语指引；重复可接受、跳过不可接受，严格区间 = 预置完成后）。
