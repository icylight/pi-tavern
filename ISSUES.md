...
## ISSUE-007: 身份可查询状态（根治「模型靠猜」）

- **状态**：open（待 User 确认形态，2026-08-01）
- **来源**：User Persona 质疑（2026-08-01）：「角色难道不是一个可以查看的状态，为什么要靠猜呢」
...
- **与 ISSUE-006 关系**：006（每轮 system prompt 身份提示）与 007 服务同一目标；007 提供确定性查证后，006 可后置/降级为可选增强（身份行 + whoami 已覆盖交互），由 User 裁决是否保留 006 独立实施。
