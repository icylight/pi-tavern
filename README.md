...
| --- | --- | --- | --- | --- |
| 软件研发 | 需求/任务 | 产品经理、软件架构师、测试工程师、开发工程师 | 产品经理扇出需求 → 软件架构师给出方案/约束、测试工程师给出验收基准 → 开发工程师实现并提交证据 → 软件架构师评审、测试工程师验收 → 结论回流产品经理 | 星形扇出 + 汇聚 |
| 故障处置 | 事件 | 事故负责人、SRE、后端工程师、数据库工程师、测试工程师 | 事故负责人统一状态并派发 → SRE/后端/数据库并行排查 → 事故负责人汇总结论 → 后端工程师修复 → 测试工程师恢复验证 → 回流事故负责人 | 中心辐射 |
| 安全审查 | 审查任务 | 安全架构师、安全工程师、开发工程师、安全测试工程师 | 安全架构师威胁建模、安全工程师代码审计并行 → 风险定级 → 开发工程师修复 → 安全工程师复验 | 对抗式回路 |
| 开源维护 | Issue/PR | Issue 分诊员、项目维护者、代码贡献者、代码评审者、发布经理 | Issue 分诊员分诊 → 项目维护者接收/指派 → 代码贡献者实现 PR → 代码评审者评审 → 发布经理发布整理 | 异步流水线 |
| 技术调研 | 调研课题 | 调研负责人、研究员、技术分析师、评审专家、文档负责人 | 研究员并行探索 → 技术分析师方案分析 → 评审专家质疑 → 调研负责人收敛结论 → 文档负责人成文 | 并行 + 收敛 |

```mermaid
flowchart LR
    PM["产品经理"] -->|"扇出需求/任务"| AR["软件架构师"] & TE["测试工程师"]
    AR -->|"方案/约束"| DV["开发工程师"]
    TE -->|"验收基准"| DV
    DV -->|"实现+证据"| AR & TE
    AR -->|"评审结论"| PM
...
```mermaid
flowchart LR
    IC["事故负责人"] -->|"统一状态/派发"| SRE["SRE"] & BE["后端工程师"] & DBE["数据库工程师"]
    SRE & BE & DBE -->|"排查结果"| IC
    IC -->|"根因+方案"| BE
    BE -->|"修复完成"| TE["测试工程师"]
    TE -->|"恢复验证"| IC
...
```mermaid
flowchart LR
    SA["安全架构师"] -->|"威胁建模+风险定级"| DV["开发工程师"]
    SE["安全工程师"] -->|"代码审计+发现清单"| DV
    DV -->|"独立修复"| SE
    SE -->|"原审计复验"| ST["安全测试工程师"]
    ST -->|"回归验证"| SA
```
