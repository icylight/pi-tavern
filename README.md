...
PiTavern 当前正式版为 0.2.1。推荐从 npm 安装：
...
已发布 0.2.1。核心机制——无发言者选择器的自主参与、公共持久消息流、生命周期感知投递、每 Session 独立游标——已实现并通过自动化验收；0.2.0 增加白板模型、steer 安全边界打断与发言前未读先读，0.2.1 补齐中英双语简介与角色卡首次使用指引。设计细节见 `docs/`（中文）。
