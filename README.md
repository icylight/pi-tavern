...
| 团队场景 | 拓扑特征 | 示例角色 | 协作模式 |
| --- | --- | --- | --- |
| 软件研发 | 扇出 → 汇聚 → 评审回流 | PM / Arch / Dev / QA | 阶段流程：计划分发 → 实现汇聚 → 评审验收 → 回流决策 |
| 故障处置 | 统一状态 + 并行排查 | 事件负责人 / 排查角色 ×N（各持独立角色卡） | 事件负责人维护统一状态；多角色并行调查 → 修复 → 恢复验证 |
| 安全审查 | 并行审计 + 对抗复验 | 威胁建模 / 代码审计 / 独立修复 / 原审计复验 | 并行审计 → 风险定级 → 独立修复 → 原角色复验（对抗式） |
| 开源维护 | Issue/PR 驱动的异步流转 | 分诊 / 维护者 / 实现 / 评审 / 发布整理 | 分诊 → 接收 → 实现 → 评审 → 发布整理（异步） |
| 技术调研 | 并行探索 → 收敛 | 研究角色 ×N（各持独立角色卡）/ 方案分析 / 反方质疑 | 并行探索 → 方案分析 → 反方质疑 → 结论收敛 |
...
flowchart LR
    subgraph 技术调研
        R1[研究 A] & R2[研究 B] & R3[研究 C] -->|并行探索| N[方案分析]
        N --> G[反方质疑]
        G --> C[结论收敛]
    end
```
