...
| 团队 | 触发 | 信息流 | 拓扑 | 关键协作约定 |
| --- | --- | --- | --- | --- |
| 软件研发 | 需求/任务 | PM 扇出 → Dev 汇聚 → Arch/QA 评审回流 | 星形扇出 + 汇聚 | 职责分工卡 + 评审/验收证据门禁 |
| 故障处置 | 事件 | 事件负责人统一状态，多排查并行 | 中心辐射 | 统一状态汇报约定（各排查角色持独立卡） |
| 安全审查 | 审查任务 | 建模/审计并行 → 定级 → 独立修复 → 原审计复验 | 对抗式回路 | 审计与修复分离 + 原角色复验 |
| 开源维护 | Issue/PR | 分诊 → 接收 → 实现 → 评审 → 发布整理 | 异步流水线 | 状态流转约定（分诊/接收/合并） |
| 技术调研 | 调研课题 | 并行探索 → 方案分析 → 反方质疑 → 收敛 | 并行 + 收敛 | 反方角色约定（红队式） |
...
```mermaid
flowchart LR
    IC["事件负责人"] -->|"统一状态"| I1["排查 A"] & I2["排查 B"] & I3["排查 C"]
    I1 & I2 & I3 -->|"调查结果"| IC
    IC -->|"根因+方案"| FX["修复"]
    FX -->|"修复完成"| VR["恢复验证"]
    VR -->|"验证通过"| IC
...
> **这些只是用户可配置的示例**——不是 PiTavern 默认内置的角色，不是已经完整验证的团队模板，也不是由扩展强制执行的状态机。（同一角色卡只能被一个 Session claim——示例中的并行角色各持独立角色卡。）
