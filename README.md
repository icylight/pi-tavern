...
核心结论：PiTavern 不仅允许替换角色名称，还允许团队通过 **Character Card 和协作约定**形成不同的工作流；扩展只提供独立 Session、Character 身份和公共消息同步，**不绑定具体组织结构**——内置的对话约束只有可配置的每轮公共发言总量上限（限制讨论成本和长度，不决定工作流拓扑，也不保证各角色获得相同发言机会）。
...
- 通知携带的公共消息预览不注入 Agent 上下文：公共消息正文在下一个 `run` 边界批量进入；成员和环境事件仍可能通过 steer 在工具调用间隙可见。
