...
### 3. 谁是卧底群聊文字局：推理对抗与信号纪律工作流

**角色**：主持人（裁判，User Persona）、代发者、玩家（多名 Character）

```mermaid
flowchart TD
    U["主持人·发词/收票/判定"] -->|"私聊发词（不带身份标签）"| P1["玩家 1"]
    U -->|"私聊发词"| P2["玩家 2"]
    U -->|"私聊发词"| P3["玩家 3"]
    P1 & P2 & P3 -->|"公屏极简描述（点名制 ≤2 信息点）"| GC["公共群聊"]
    U -->|"公屏信号（裁判：前缀）"| GC
    P1 & P2 & P3 -->|"私聊投票"| U
    U -->|"结算/判定（降代发者原话转述）"| GC
    GC -->|"点名/信号只读响应"| P1 & P2 & P3
```

主持人私聊给每位玩家发词（**不带身份标签**——身份未知局，玩家靠描述互证判断自己是平民还是卧底）→ 玩家轮流公屏用一句话描述自己的词（**极简描述：每句 ≤2 个信息点**，假想自己是卧底来发言）→ 私聊投票 → 主持人公屏结算并判定胜负（卧底被投出可猜词翻盘——**仅限第 1 轮**；存活降至 2 人卧底即胜）。信号统一「裁判：」前缀、点名制发言、群聊讨论轮次 = 游戏轮次（额度自动重置）。这是同一套机制（独立 Session、身份隔离、公共消息同步）的**对抗推理用法**——桌面推理游戏的完全文字化，零代码实现，规则见 `docs/who-is-spy.md`（含演练局六局实测教训与验收结论）。
