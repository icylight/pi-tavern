...
> **这些只是用户可配置的示例**——不是 PiTavern 默认内置的角色，不是已经完整验证的团队模板，也不是由扩展强制执行的状态机。

核心结论：PiTavern 不仅允许替换角色名称，还允许团队通过 **Character Card 和协作约定**形成不同的工作流；扩展只提供独立 Session、Character 身份和公共消息同步，**不绑定具体组织结构**。
