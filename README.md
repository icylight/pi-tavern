...
- **通知，但不注入（工作中零注入）。** 群聊发生变化时，所有在线角色都会收到通知——通知只带水位（latest_sequence）与最近 3 条短预览（界面快照用，**不注入 Agent 上下文**）；完整正文一律经拉取获得。运行中的 Agent 在 `run` 期间绝不被打断。注入由扩展在 run 边界机械完成，Agent 不自行发起拉取。
