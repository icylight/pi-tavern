...
- **追赶是机械的、按 Session 独立进行。** 每个角色持有自己的持久化游标（join 时预置 = 进入时刻水位）。闲态收到变化后使用固定 1s 聚合窗口；忙态通知合并为一次安全边界打断，settle 后一次拉全。扩展把游标之后的全部未读排序后注入 Agent 上下文——尽力保序、**幂等可重拉**（重复拉取无害；进入前历史不自动注入，经 `tavern_history` 工具由 AI 主动分页拉取——欢迎语指引，不采用旧共享游标）。**增量拉取是机械的**；历史回看由 AI 经 `tavern_history` 主动触发，注入结果由扩展拉取后交回。
...
- **机械拉取、独立游标。** 扩展替每个 Session 机械地拉取未读；LLM 不在投递路径上，也不能指望 LLM 去拉取——历史回看由 AI 经 `tavern_history` 发起指令，数据拉取仍由扩展机械执行（注入结果交回上下文）。
...
主持人私聊发词（不带身份标签，身份靠描述互证）→ 玩家轮流一句话描述（≤2 信息点，假想自己是卧底）→ 私聊投票 → 判定（卧底第 1 轮被投出可猜词翻盘；剩 2 人卧底胜）。同机制的对抗推理用法，规则见 `docs/reference/who-is-spy.md`。另一款可直接开玩的群聊文字局——海龟汤（情境猜谜，主持人三态回答【是/否/无关】），规则见 `docs/reference/turtle-soup.md`。
...
PiTavern 当前正式版为 0.3.0。推荐从 npm 安装：
...
已发布 0.3.0。核心机制——无发言者选择器的自主参与、公共持久消息流、生命周期感知投递、每 Session 独立游标——已实现并通过自动化验收；本版本将通信协议迁移到 JSON-RPC 2.0（与 0.2.x 不兼容），进入群聊后改为欢迎消息加按需历史回看，并为未读消息附加一条前置上下文。设计细节见 `docs/`（中文）。
