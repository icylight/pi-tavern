 ...
 *
 * 契约源：issue #114（09:50 版）B6 节 + 验收锚点 1-4 + ADR-0007。
 *
 * 验收锚点挂靠：
...
describe("acceptance: #114 whiteboard board flow e2e", () => {
	...
	interface BoardWriteResponse {
		...
		command: "board_write";
		success: boolean;
		data: { changed: boolean; note?: { id: string; content: string }; code?: string };
	}
	...
	interface BoardQueryResponse {
		...
		command: "board_query";
		success: boolean;
		data: { boards: Record<string, { id: string; content: string }[]> };
	}
	...
	it("board 全流程：贴/改/撕/清 + 四态 + 广播 + 静默 + status 小节（锚点 ④）", async () => {
		...
		try {
			// ── 贴新条：id 回带 + add 广播 ─────────────────────────
			client.send({ id: "w1", type: "board_write", action: "set", note: { content: "README 放仓库根目录" } });
			const r1 = asBoardWriteResponse(
				await client.waitFor((m) => m.type === "response" && m.command === "board_write" && m.id === "w1"),
			);
			expect(r1.success).toBe(true);
			expect(r1.data.changed).toBe(true);
			expect(typeof r1.data.note?.id).toBe("string");
			expect(r1.data.note?.content).toBe("README 放仓库根目录");
			const r1note = r1.data.note;
			if (r1note === undefined) throw new Error("B1 定稿：set 新贴响应 note 必带（id 回带闭环）");
			const noteId = r1note.id as string;

			const u1 = asBoardUpdate(await client.waitFor((m) => m.type === "board_update"));
			expect(u1.actor).toBe("characters/architect.md");
			expect(u1.action).toBe("add");
			expect(u1.note).toEqual({ id: noteId, content: "README 放仓库根目录" });
			...
			const r2 = asBoardWriteResponse(
				await client.waitFor((m) => m.type === "response" && m.command === "board_write" && m.id === "w2"),
			);
			expect(r2.data.changed).toBe(true);
			expect(r2.data.note?.content).toBe("README 放根目录（v2）");
			await client.waitFor(
				(m) =>
					m.type === "board_update" && m.action === "update" && (m.note as { id: string } | undefined)?.id === noteId,
			);

			// ── 撕条：remove 广播携带被撕条完整内容（锚点 ③ 增量摘要）──
			client.send({ id: "w3", type: "board_write", action: "remove", note: { id: noteId } });
			const r3 = asBoardWriteResponse(
				await client.waitFor((m) => m.type === "response" && m.command === "board_write" && m.id === "w3"),
			);
			expect(r3.data.changed).toBe(true);
			const u3 = asBoardUpdate(await client.waitFor((m) => m.type === "board_update" && m.action === "remove"));
			expect(u3.note).toEqual({ id: noteId, content: "README 放根目录（v2）" });

			// ── remove 不存在条：告知码 + 群聊静默（拍板 ③）────────
			const ghostIndex = client.allFrames().length;
			client.send({ id: "w4", type: "board_write", action: "remove", note: { id: "no-such-id" } });
			const r4 = asBoardWriteResponse(
				await client.waitFor((m) => m.type === "response" && m.command === "board_write" && m.id === "w4"),
			);
			expect(r4.data.changed).toBe(false);
			expect(r4.data.code).toBe("note_not_found");
			...
			for (let i = 0; i < 5; i += 1) {
				const tag = `n${i}`;
				client.send({ id: `w5-${i}`, type: "board_write", action: "set", note: { content: tag } });
				const ri = asBoardWriteResponse(
					await client.waitFor((m) => m.type === "response" && m.command === "board_write" && m.id === `w5-${i}`),
				);
				expect(ri.data.changed).toBe(true);
				ids.push((ri.data.note as { id: string }).id);
			}
			// 等最后一条 add 广播落帧（响应先到、广播后到——rejectIndex 须在广播之后取）
			await client.waitFor(
				(m) => m.type === "board_update" && m.action === "add" && (m.note as { id: string } | undefined)?.id === ids[4],
			);
			const rejectIndex = client.allFrames().length;
			client.send({ id: "w6", type: "board_write", action: "set", note: { content: "第六条" } });
			const r6 = asBoardWriteResponse(
				await client.waitFor((m) => m.type === "response" && m.command === "board_write" && m.id === "w6"),
			);
			expect(r6.data.changed).toBe(false);
			expect(r6.data.code).toBe("max_notes_exceeded");
			await waitNoBoardUpdate(client, rejectIndex);

			// ── clear 非空板：applied 广播 clear 无 note ───────────
			client.send({ id: "w7", type: "board_write", action: "clear" });
			const r7 = asBoardWriteResponse(
				await client.waitFor((m) => m.type === "response" && m.command === "board_write" && m.id === "w7"),
			);
			expect(r7.data.changed).toBe(true);
			expect(r7.data.note).toBeUndefined();
			await client.waitFor((m) => m.type === "board_update" && m.action === "clear");
			expect((client.allFrames().at(-1) as Record<string, unknown>).note).toBeUndefined();

			// ── clear 空板：board_empty 告知码 + 静默 ───────────────
			const emptyIndex = client.allFrames().length;
			client.send({ id: "w8", type: "board_write", action: "clear" });
			const r8 = asBoardWriteResponse(
				await client.waitFor((m) => m.type === "response" && m.command === "board_write" && m.id === "w8"),
			);
			expect(r8.data.changed).toBe(false);
			expect(r8.data.code).toBe("board_empty");
	...
	it("字符侧消费：门闸放行 + 白板桶渲染可达 + 不触发消息流拉取（B4 接线 e2e）", async () => {
		...
		try {
			...
			client.send({ id: "b1", type: "board_write", action: "set", note: { content: "共识一" } });
			asBoardWriteResponse(
				await client.waitFor((m) => m.type === "response" && m.command === "board_write" && m.id === "b1"),
			);
	...
	it("收敛重放：README 位置议题立场翻转——撕旧贴新、板上只有当前、无陈旧引用（锚点 ①②③）", async () => {
		...
		try {
			...
			async function postStance(client: BufferedWsClient, id: string, content: string, note?: { id: string }) {
				client.send({ id, type: "board_write", action: "set", note: note ? { id: note.id, content } : { content } });
				const r = asBoardWriteResponse(
					await client.waitFor((m) => m.type === "response" && m.command === "board_write" && m.id === id),
				);
			...
			async function tear(client: BufferedWsClient, id: string, noteId: string) {
				client.send({ id, type: "board_write", action: "remove", note: { id: noteId } });
				const r = asBoardWriteResponse(
					await client.waitFor((m) => m.type === "response" && m.command === "board_write" && m.id === id),
				);
				expect(r.data.changed).toBe(true);
			...
			// ── 机制断言：板上只有当前立场（锚点 ①②③）──────────
			arch.send({ id: "q1", type: "board_query" });
			const q = asBoardQueryResponse(
				await arch.waitFor((m) => m.type === "response" && m.command === "board_query" && m.id === "q1"),
			);
