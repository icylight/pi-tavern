...
describe("WH7: whisper 校验后投递失败不回滚（窄窗口竞态）", () => {
	...
	it("chain: whisper delivery-failure no-rollback——目标连接非 OPEN 时真实 BroadcastHub 组合下调用仍成功 + 已提交不回滚（WH7 主锚，可证伪）", async () => {
			...
			(socket, frame) => {
				const hub = new BroadcastHub({
					...
					iterateConnections: (visit) => {
						for (const s of connections.values()) visit(s);
					},
					isActive: () => true,
					onSendFailure,
				...
				hub.send(socket, frame);
			},
		);
		connections.set("target-session", targetSocket);
		...
		// 投递失败被静默容错 → 调用仍成功返回（WH7：不回滚、不重试、不占二次额度）。
		...
		// 可证伪关键断言：目标 socket 非 OPEN → socket.send 未被调用 + onSendFailure 触发
		// （证明「发送确实失败且被静默容错」——而非与成功路径行为等价）。
		expect(targetSocket.send).not.toHaveBeenCalled();
		expect(onSendFailure).toHaveBeenCalledWith(targetSocket);
