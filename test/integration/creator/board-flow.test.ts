/**
 * B3 白板管线集成测试（#114，ADR-0007 §3；挂靠 issue 09:26 版 B3 节 + QA 清单 1-5）。
 *
...
const isBoardUpdate = (m: Record<string, unknown>): boolean => m.type === "board_update";
const isBoardWriteResponse = (m: Record<string, unknown>): boolean =>
	m.type === "response" && m.command === "board_write";
...
describe("B3 白板管线（#114，integration）", () => {
	it("贴条：响应四态 applied 带 note（id 回带）+ 广播 board_update add", async () => {
		...
		peer.send({ id: "w1", type: "board_write", action: "set", note: { content: "第一条" } });
		...
		const update = await peer.waitFor((m) => isBoardUpdate(m) && m.action === "add");
		expect(update.actor).toBe(characters[0].characterId);
		expect(update.note).toEqual({ id: note.id, content: "第一条" });
		// 无 sequence 字段（不在消息流、无水位语义）。
		expect("sequence" in update).toBe(false);
	});
	...
	it("改条/撕条/清板增量摘要：update·remove 带内容、clear 无 note；N 条 applied → N 条广播", async () => {
		...
		peer.send({ id: "w1", type: "board_write", action: "set", note: { content: "待改" } });
		const r1 = await peer.waitFor((m) => isBoardWriteResponse(m) && m.id === "w1");
		const note1 = (r1.data as Record<string, unknown>).note as Record<string, unknown>;

		// edit → action update（响应带新内容，广播同）
		peer.send({ id: "w2", type: "board_write", action: "set", note: { id: note1.id, content: "改后" } });
		const r2 = await peer.waitFor((m) => isBoardWriteResponse(m) && m.id === "w2");
		expect((r2.data as Record<string, unknown>).changed).toBe(true);
		const u2 = await peer.waitFor((m) => isBoardUpdate(m) && m.action === "update");
		expect(u2.note).toEqual({ id: note1.id, content: "改后" });

		// remove → 广播携带被撕条完整内容（id + content）；响应不带 note
		peer.send({ id: "w3", type: "board_write", action: "remove", note: { id: note1.id } });
		const r3 = await peer.waitFor((m) => isBoardWriteResponse(m) && m.id === "w3");
		expect((r3.data as Record<string, unknown>).changed).toBe(true);
		expect((r3.data as Record<string, unknown>).note).toBeUndefined();
		const u3 = await peer.waitFor((m) => isBoardUpdate(m) && m.action === "remove");
		expect(u3.note).toEqual({ id: note1.id, content: "改后" });

		// 再贴一张 → clear（非空板 applied，广播无 note）
		peer.send({ id: "w4", type: "board_write", action: "set", note: { content: "第二张" } });
		await peer.waitFor((m) => isBoardWriteResponse(m) && m.id === "w4");
		peer.send({ id: "w5", type: "board_write", action: "clear" });
		const r5 = await peer.waitFor((m) => isBoardWriteResponse(m) && m.id === "w5");
		expect((r5.data as Record<string, unknown>).changed).toBe(true);
		expect((r5.data as Record<string, unknown>).note).toBeUndefined();
		const u5 = await peer.waitFor((m) => isBoardUpdate(m) && m.action === "clear");
		expect(u5.note).toBeUndefined();

		// wire 层即时广播：5 次 applied → 5 条 board_update（顺序一致，无合并）
		const updates = peer.allFrames().filter((m) => isBoardUpdate(m));
		expect(updates.map((m) => m.action)).toEqual(["add", "update", "remove", "add", "clear"]);
	});
	...
	it("告知/拒绝：changed:false 群聊静默（不广播 board_update）", async () => {
		...
		peer.send({ id: "w1", type: "board_write", action: "remove", note: { id: "ghost" } });
		const r1 = await peer.waitFor((m) => isBoardWriteResponse(m) && m.id === "w1");
		expect(r1.data).toEqual({ changed: false, code: "note_not_found" });
		...
		for (let i = 0; i < 5; i++) {
			peer.send({ id: `ws${i}`, type: "board_write", action: "set", note: { content: `条${i}` } });
	...
	it("actor 隔离：A 用 B 的条 id = note_not_found，双板互不串写", async () => {
		...
		a.send({ id: "wa", type: "board_write", action: "set", note: { content: "A 的" } });
		await peerBoardWriteResponse(a, "wa");

		b.send({ id: "wb", type: "board_write", action: "set", note: { content: "B 的" } });
		await peerBoardWriteResponse(b, "wb");

		// 先查 B 的条 id（隔离断言需要 B 的真实 id）
		a.send({ id: "wq", type: "board_query" });
		const query = await peerBoardQuery(a, "wq");
		const boards = (query.data as Record<string, unknown>).boards as Record<string, unknown>;
		...
		a.send({ id: "wx2", type: "board_write", action: "remove", note: { id: noteB.id } });
		const rx = await peerBoardWriteResponse(a, "wx2");
		expect(rx.data).toEqual({ changed: false, code: "note_not_found" });

		// 双板不变
		a.send({ id: "wq2", type: "board_query" });
		const q2 = await peerBoardQuery(a, "wq2");
		const boards2 = (q2.data as Record<string, unknown>).boards as Record<string, unknown>;
	...
	it("board_write 不占发言额度、不产生消息流与 group_chat_update", async () => {
		...
		// 基线 round 快照
		peer.send({ id: "g1", type: "get_group_chat_state" });
		const before = await peer.waitFor(
			(m) => m.type === "response" && m.command === "get_group_chat_state" && m.id === "g1",
		);
		const roundBefore = (before.data as Record<string, unknown>).round as Record<string, unknown>;

		const groupChatUpdateBefore = peer.allFrames().filter((m) => m.type === "group_chat_update").length;

		// 两次贴条 + 一次撕条
		peer.send({ id: "w1", type: "board_write", action: "set", note: { content: "贴" } });
		await peer.waitFor((m) => isBoardWriteResponse(m) && m.id === "w1");
		peer.send({ id: "w2", type: "board_write", action: "set", note: { content: "贴2" } });
		await peer.waitFor((m) => isBoardWriteResponse(m) && m.id === "w2");

		// round 计数不变（无额度消耗）
		peer.send({ id: "g2", type: "get_group_chat_state" });
		const after = await peer.waitFor(
			(m) => m.type === "response" && m.command === "get_group_chat_state" && m.id === "g2",
		);
		const roundAfter = (after.data as Record<string, unknown>).round as Record<string, unknown>;
		expect(roundAfter).toEqual(roundBefore);

		// 无消息流产出（无 public_message / message_history 增量）
		expect(peer.allFrames().filter((m) => m.type === "public_message")).toHaveLength(0);

		// 无 group_chat_update 通知（board 走独立广播通道，不触发消息流拉取）
		await new Promise((resolveSleep) => setTimeout(resolveSleep, 400));
		expect(peer.allFrames().filter((m) => m.type === "group_chat_update")).toHaveLength(groupChatUpdateBefore);
	});
	...
	it("leave → resume → board_query 读回（文件保留；删除群聊同步清理）", async () => {
		...
		peer.send({ id: "w1", type: "board_write", action: "set", note: { content: "持久化" } });
		const r1 = await peer.waitFor((m) => isBoardWriteResponse(m) && m.id === "w1");
		const note = (r1.data as Record<string, unknown>).note as Record<string, unknown>;

		// 补首条消息建轮次 + speak 落会话条目（board 操作不入消息流/不写会话文件——resume 需会话文件存在）
		await runtime.submitUserPersonaMessage("开题");
		peer.send({ id: "sp1", type: "speak", content: "留档发言" });
		await peer.waitFor((m) => m.type === "response" && m.command === "speak" && m.id === "sp1");

		peer.send({ type: "leave_group_chat" });
		await runtime.close();
		await runtime.close();
		...
		peer2.send({ id: "q1", type: "board_query" });
		const query = await peer2.waitFor((m) => m.type === "response" && m.command === "board_query" && m.id === "q1");
		const boards = (query.data as Record<string, unknown>).boards as Record<string, unknown>;
	...
	it("协议级错误走 sendFailure（未入群成员直接 board_write）", async () => {
		...
		peer.send({ id: "w1", type: "board_write", action: "set", note: { content: "未入群" } });
...
async function peerBoardQuery(peer: Peer, id: string): Promise<Record<string, unknown>> {
	return peer.waitFor((m) => m.type === "response" && m.command === "board_query" && m.id === id);
}
