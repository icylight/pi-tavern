...
### Schema 单一事实源（#145 文档生成化）

**全部字段形状（信封/方法/参数/响应）以生成产物为权威**：

- 客户端消息（request/notification 判别联合）：[docs/protocol/schema/ClientMessage.json](../protocol/schema/ClientMessage.json)
- 服务端消息（响应/广播/通知判别联合）：[docs/protocol/schema/ServerMessage.json](../protocol/schema/ServerMessage.json)
...
公开的 Character 摘要来自角色卡 frontmatter（字段形状：[CharacterSummary.json](../protocol/schema/CharacterSummary.json)）：
...
在线 Character 在摘要基础上增加 `is_self` / `is_streaming` / `hand_raised`（字段形状：[OnlineCharacter.json](../protocol/schema/OnlineCharacter.json)）：
...
字段形状：[ClientMessage.json](../protocol/schema/ClientMessage.json) `join_group_chat` 分支；`params.session_id` = 当前 pi session 的 ID。

群聊创建者使用 `session_id` 防止同一个当前 pi session 建立重复成员连接。首版不提供断线重连或成员恢复分支。

加入响应：[ServerMessage.json](../protocol/schema/ServerMessage.json) `join_group_chat` 响应分支（`result.available_characters` = 角色摘要数组）。
...
字段形状：[ClientMessage.json](../protocol/schema/ClientMessage.json) `claim_character` 分支。

群聊创建者按照消息到达顺序原子检查并预留 Character（原子：检查与预留作为一个整体一次完成，不出现中间可见态）。已经预留或已经在线的 Character 不能再次预留。成功响应：[ServerMessage.json](../protocol/schema/ServerMessage.json) `claim_character` 响应分支（`result.character` 含 `path` = 角色卡本机绝对路径）。
...
Character 已经被预留或已经在线等失败情况使用 pi-coding-agent 风格的通用错误响应（形状：[ProtocolErrorObject.json](../protocol/schema/ProtocolErrorObject.json)）：
...
本地角色卡读取、验证和 `CharacterRuntime` 准备成功后，加入方在同一连接上发送 `character_ready`（字段形状：[ClientMessage.json](../protocol/schema/ClientMessage.json) `character_ready` 分支，`params` 恒为空对象）。

请求不携带 `character_id` 或 `session_id`。群聊创建者根据连接闭包中保存的预留确定身份。没有有效预留、预留已释放或连接已经在线时返回通用错误响应。

成功响应：[ServerMessage.json](../protocol/schema/ServerMessage.json) `character_ready` 响应分支（`result.latest_sequence` 或 `result: null`）。
...
Character 完成 `character_ready` 并正式成为群成员后，群聊创建者向此时的全部在线 Character 广播 `character_joined`（字段形状：[ServerMessage.json](../protocol/schema/ServerMessage.json) `character_joined` 通知分支，`params.character` = 公开摘要）。
- 广播遵循协议的统一广播语义。
...
Character 主动离开请求（字段形状：[ClientMessage.json](../protocol/schema/ClientMessage.json) `leave_group_chat` 分支，`params` 恒为空对象）。
...
成功响应：[ServerMessage.json](../protocol/schema/ServerMessage.json) `leave_group_chat` 响应分支（`result: null`）。

离开广播：`character_left` 通知（[ServerMessage.json](../protocol/schema/ServerMessage.json) 对应分支；`params.reason` 二值见下）。
`reason` 首版支持：
...
关闭广播：`group_chat_closed` 通知（字段形状：[ServerMessage.json](../protocol/schema/ServerMessage.json) 对应分支，`params.group_chat_id`）。
...
#123 起 join/ready 不再自动推送历史（改为单播 `system_message` 欢迎语）；历史全部经主动查询获取。Character 需要历史时发送 `get_message_history`（字段形状：[ClientMessage.json](../protocol/schema/ClientMessage.json) `get_message_history` 分支，`params.cursor` 可选）。

响应：[ServerMessage.json](../protocol/schema/ServerMessage.json) `get_message_history` 响应分支（`result.messages` / `cursor` / `has_more` / `total_messages`）。
...
#123：`character_ready` 成功后，群聊创建者向新 Character **单播**一条 `system_message` 欢迎语（不再自动推送历史；字段形状：[ServerMessage.json](../protocol/schema/ServerMessage.json) `system_message` 通知分支，`params` 仅 `content`）：
...
请求（字段形状：[ClientMessage.json](../protocol/schema/ClientMessage.json) `get_chat_history_file` 分支，`params` 恒为空对象）。

响应：[ServerMessage.json](../protocol/schema/ServerMessage.json) `get_chat_history_file` 响应分支（`result.path`）。
...
请求（字段形状：[ClientMessage.json](../protocol/schema/ClientMessage.json) `get_group_chat_state` 分支，`params` 恒为空对象）。

响应：[ServerMessage.json](../protocol/schema/ServerMessage.json) `get_group_chat_state` 响应分支（`result.group_chat` / `result.round` / `result.online_characters`，各字段语义见下）。
...
Character 上报（字段形状：[ClientMessage.json](../protocol/schema/ClientMessage.json) `update_character_state` 通知分支，`params.is_streaming`）：
...
Character 的 `tavern_speak` Agent tool 通过 WebSocket 请求原子尝试发布完整内容（字段形状：[ClientMessage.json](../protocol/schema/ClientMessage.json) `speak` 分支，`params.content` + 可选 `params.based_on_sequence`）：
...
成功发布：[ServerMessage.json](../protocol/schema/ServerMessage.json) `speak` 响应成功分支（`published: true` + `event_id` / `sequence` / `latest_sequence` / `round`）。

- `latest_sequence`（ISSUE-013）：发布后服务端最新序号（成功时等于本次 `sequence`）。**纯信息字段、非推进源**——客户端不据此推进游标或任何已见序号（B6 由服务端排除自身判定保证不误拒），客户端不得依赖该字段做状态推进。

额度耗尽：[ServerMessage.json](../protocol/schema/ServerMessage.json) `speak` 响应 `round_limit_reached` 分支（`published: false` + `hand_raised: true` + `round`）。
...
协议错误或连接身份错误使用 `error` 响应（形状：[ProtocolErrorObject.json](../protocol/schema/ProtocolErrorObject.json)）。

### 公开消息广播与增量拉取（M7/ISSUE-012)

#### `public_message`（历史/拉取消息形态）

User Persona 和 Character 的公开消息统一使用 `public_message` 结构（字段形状：[ServerMessage.json](../protocol/schema/ServerMessage.json) `public_message` 通知分支；`sender` 按类型判别 `user_persona` / `character`，`source` 缺省 = `group`）。`group_chat_update.preview_messages`、`message_history.messages` 与 `fetch_messages_since` 的响应均复用此结构。
...
公开消息的广播以通知形式发出，不再逐条推送完整消息。v0.5 功能收窄后，本通知只由成功持久化的公开消息触发；白板使用独立 `board_update`，成员与流式状态变化不触发本通知（字段形状：[ServerMessage.json](../protocol/schema/ServerMessage.json) `group_chat_update` 通知分支；`preview_messages` = 最近 3 条 `public_message`）。
...
请求（字段形状：[ClientMessage.json](../protocol/schema/ClientMessage.json) `fetch_messages_since` 分支，`params.since_sequence` 必带）。

成功响应：[ServerMessage.json](../protocol/schema/ServerMessage.json) `fetch_messages_since` 响应分支（`result.messages` = `sequence > since_sequence` 的全部公开消息 + `latest_sequence` + `total_messages`）。
