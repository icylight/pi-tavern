...
- 客户端消息（request/notification 判别联合）：[client.jsonc](../../src/protocol/schema/client.jsonc) `ClientMessage` 定义
- 服务端消息（响应/广播/通知判别联合）：[server.jsonc](../../src/protocol/schema/server.jsonc) `ServerMessage` 定义
- 白板模型：[board.jsonc](../../src/protocol/schema/board.jsonc)（`BoardNote` / `BoardReasonCode` / `BoardWriteData` / `BoardWriteResponse` / `BoardQueryResponse` / `BoardUpdate`）
- 跨帧共享摘要/信封：[common.jsonc](../../src/protocol/schema/common.jsonc)（`CharacterSummary` / `OnlineCharacter` / `RequestId` / `ProtocolErrorObject` / `NullResult` / `RoundSnapshot`）

生成链：`src/protocol/schema/*.jsonc`（唯一手写处）→ `scripts/generate-schema.mjs`（翻译器）→ `src/protocol/generated/schema.ts`（程序消费的 TypeBox schema，**请勿手改**）；codec 校验与 `Static<typeof>` 类型均从生成产物同源派生。本文档其余部分**只描述时序、语义与边界**，不再内联字段形状；文中的 JSON 仅为语义示意，字段以 schema 定义文件为准。
