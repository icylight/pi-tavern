...
1. **ADR-0005 §3「一致性边界（无 DB 的事务对应物）」表述**（User 评论 r3698822940「这部分太奇怪了」，2026-08-02，不阻塞）：给文件持久化强套「事务对应物」框架，架构词藻重于代码事实。处置：Phase 1（skills 提取）落地时以实际代码机制重写——session-store 的真实一致性（append 原子写 + FIRST_PERSIST_* 恢复 + 游标单调推进）由代码事实定义，删除事务类比；ADR 转 Accepted 前必须完成
