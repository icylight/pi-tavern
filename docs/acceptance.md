...
1. **A1 增量拉取（空闲期立即 / 活跃期聚合）**：空闲期收到 `group_chat_update` → 立即发起 `fetch_messages_since(游标)`，无 1s 防抖（单测 fake timers 精确控时断言）；**活跃期（agent 运行中）→ ≤400ms 固定窗口聚合**：窗口内多条 update 合并为一次拉取 + 一次投递（打断 N→1），onSettled 兜底尾部（光标单调不重不漏）；窗口形态 = 固定窗口非滑动（延迟上界 ≤ 首消息后 400ms，契约修订 #60，2026-08-02）；
