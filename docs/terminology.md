...
| Host | `Host` / Host Pi | 承载当前群聊通信、调度、持久化和用户界面的 Pi 进程。 |
| 用户 | `User` | 在 Host 中发送公共消息的人，不是角色或群成员。 |
| 角色 | `Character` | 由角色卡定义的身份，不等同于某个 Pi 进程。 |
| 角色卡 | `CharacterCard` / Character Card | 使用 Markdown 定义角色提示词的文件。 |
| 群成员 | `GroupMember` / Group Member | 已加入当前群聊并领取角色卡的 Pi。 |
...
- 角色 Pi 执行 `/tavern-leave` 表示退出群聊；Host 执行该命令表示关闭当前群聊。
