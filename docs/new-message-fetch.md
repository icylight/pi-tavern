...
## 7. 验收方向（待量化后写入 acceptance.md）

- 广播到达后，角色在无防抖约束下发起增量拉取；拉取内容从持久化游标之后开始，不重不漏、顺序一致
- 当前 run 正在执行时收到广播：run 不被中断；run 结束 → 新消息进入角色上下文，间隔 ≤ X 秒（X 待定，建议 5s）
- 重启角色 session 后游标保留，重新 join 后从游标处差分同步（无游标时全量分页）
- 缺口场景（广播丢失/序号跳跃）：角色最终能补齐消息，不永久丢失
