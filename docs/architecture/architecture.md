...
│   └── reload-handoff-registry.ts  runtime·进程级 reload 交接
...
│   ├── broadcast-hub.ts       runtime·通知投递（N→1 聚合）
...
│   ├── runtime-lifecycle.ts   runtime·生命周期域（含 watchdog）
...
| 位置 | IO | 豁免依据 |
| --- | --- | --- |
| `config/load-config.ts` + `character-card.ts` | readFile / readdir / realpath / stat | 组合根唯一 loadTavernConfig，装配与行为分离 |
| `creator/creator-factory.ts` | statSync / rm / writeFile | lint-layers 白名单：组合根装配语义（默认依赖装配） |

grep 歧义排除：`grep writeFile` 额外命中 `creator-runtime.ts:88` / `reload-flow.ts:39` 的 `writeFile` 字样为**依赖注入接口类型签名**（import 无 node:fs），非直连 IO 调用点。

## 6. 挂起项（登记于 architecture-backlog，此处仅列现状）

| 项 | 五层决策目标 | 现状 | 状态 |
| --- | --- | --- | --- |
| character-pipelines/ | application 层：发言策略 + steer 策略，自 group-chat-input.ts 拆出 | 无该目录；group-chat-input.ts 612 行，steer 实现在内 | 挂起（待内嵌） |
| character-runtime 瘦身 | 768 行同理拆分 | 852 行未拆 | 挂起（待内嵌） |
...
4. 挂起项开工（character-pipelines 拆分 / character-runtime 瘦身）：以 §6 挂起项与 `docs/development/architecture-backlog.md` 为起点，遵循行为等价基准（先钉后迁：迁移 PR 内先写行为钉测 → 迁移 → 断言零改动仍绿）
