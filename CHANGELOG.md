...
- 协议层迁移 JSON-RPC 2.0 标准信封（#119）：判别字段 `type` → `method`、载荷进 `params`、响应改 `{result}`/`{error}`；请求/响应必带 id（无 id 拒绝）；错误码纳入 JSON-RPC 标准码。wire 契约变更——随 0.3.0 发布，与旧版客户端不兼容（一次性豁免零漂移，ADR-0006）。
