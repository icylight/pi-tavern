...
- **不兼容变更**：协议层迁移 JSON-RPC 2.0 标准信封（#119），判别字段 `type` → `method`、载荷进 `params`、响应改 `{result}`/`{error}`；请求/响应必带 id（无 id 拒绝）；错误码纳入 JSON-RPC 标准码。0.2.x 客户端无法与 0.3.0 混用（一次性豁免零漂移）。
...
- 白板模型（#114）：头脑风暴状态收敛机制——每角色一块自己的白板（贴条/撕条/清板/查询），全群可见，更新发增量摘要通知（「喊一声」），默认 5 条 / 140 码点（可配置），随群聊删除同步清理。
...
- 五层架构重构 Phase 2+3 完成：application 层六管线门面化 + runtime 瘦身（creator-runtime 1881→429 行骨架，拆出十模块）+ index.ts 唯一装配点（#58/#69/#72）。
...
- 群聊消息数量上限 10 → 100（#30）。
- 五层架构重构 Phase 1：resume-projection / discovery / session·cursor store 归位 `src/data/`（skills 层），行为零变化、契约零 diff（#55/#59/#61/#63）。
