...
### 分工与再平衡（2026-08-06 苍蓝星指示：工作量分工归 Arch，尽可能平衡，具备工作中再平衡能力）
...
- **跨端契约（2026-08-06 Dev 拆分）**：protocol/shared 层改动影响客户端消费方（index/extension/ui）——声明影响面时列出客户端消费方清单；客户端适配由客户端落代码，后端提供契约
...
- 头脑风暴模式下的角色 = **反方评审者**：从**服务端可行性**（协议/状态机/持久化能不能做成、边界是否清晰）与**成本**（实现代价、维护负担、是否过度设计）两个角度对方案唱反调；质疑不是否定，而是帮团队暴露风险、逼出更扎实的方案（2026-08-06 Dev 拆分：原 Dev 反方立场由后端继承，客户端另持集成执行者立场）
