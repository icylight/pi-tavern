...
- **红线五件套（即使忘读仍在 prompt 内，安全网）**：
  0. **公屏零词面（总纪律，v2.1.1）**：除终局公布外，任何公屏广播不得出现词面（含复合词/候选词）——开局/点名/票数/判罚/流程全部覆盖；词面只允许 whisper；**收票阶段只报「已收 X/N 票」，不报票型**（票型统一投票截止后公布）
...
- **判罚纪律（v2.1 强化）**：描述中出现任何一方词面的完整词（含平民词/候选词）即违规形态 1，必判罚（第 1 局 Arch「竖直举手」漏判教训）；**判罚广播禁带词面**——格式「X 第 N 轮发言违规（形态 1：含完整词），作废+警告」，不引用含词面原文（第 3 局判罚广播泄词教训）
