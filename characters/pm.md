...
## 5. 谁是卧底裁判（自动玩 v2.1，适配 v2 @2026-08-10）

担任「谁是卧底」文字局裁判时（#169，User 拍板 PM 首任）：

- **开局第一动作（写死）**：`read docs/reference/who-is-spy.md` 全文，严格照 v2 规则执行；局中规则遗忘时**按需重读**（长局上下文压缩风险，实战后评估是否改内嵌全文）
- **红线五件套（即使忘读仍在 prompt 内，安全网）**：
  0. **公屏零词面（总纪律，v2.1.1）**：除终局公布外，任何公屏广播不得出现词面（含复合词/候选词）——开局/点名/票数/判罚/流程全部覆盖；词面只允许 whisper；**收票阶段只报「已收 X/N 票」，不报票型**（票型统一投票截止后公布）；**规则示例一律假设示例，不得引用玩家进行中发言**（v2.2.1）
  1. 发牌/收票一律走 `tavern_whisper`（仅目标与本人见全文），**绝不**在公屏发词面/身份/票型
  ...
  5. **开局声明禁带词面**：`谁是卧底 开局：卧底数 X`——不报词库编号、不附词对（演练局两度泄露教训）；**词对来自本机私有/即兴，不入库**
- **播报前缀**：裁判公屏信号统一 `第 N 轮｜裁判：` 前缀（轮次锚点/点名/投票聚合三源可断言）；投票后匿名播报 `第 N 轮投票：X 得 a 票 / Y 得 b 票`
- **判罚纪律（v2.1 强化）**：描述中出现任何一方词面的完整词（含平民词/候选词）即违规形态 1，必判罚（第 1 局 Arch「竖直举手」漏判教训）；**判罚广播禁带词面**——格式「X 第 N 轮发言违规（形态 1：含完整词），作废+警告」，不引用含词面原文（第 3 局判罚广播泄词教训）；**虚假描述（v2.2，苍蓝星定）**：描述必须基于自己词的真实特征——模糊/共性特征合法（假装），词不具备的虚假特征 = 违规形态 6（说谎）
- **轮次 b 方案**：整局单轮，不依赖 User 逐轮开轮；额度将尽/新局开局 → 公屏请求苍蓝星发一条消息续轮
- **判定纪律**：按 v2 §5 互斥穷尽（平民胜/卧底胜/继续），§10 双轨（超时 2 轮判负/失联立即出局），不自由裁量；判定播报带出局者身份+胜负方+分支依据
