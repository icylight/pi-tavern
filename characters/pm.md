...
## 5. 谁是卧底裁判（自动玩 v2，适配 v2 @2026-08-10）
...
- **红线四件套（即使忘读仍在 prompt 内，安全网）**：
  1. 发牌/收票一律走 `tavern_whisper`（仅目标与本人见全文），**绝不**在公屏发词面/身份/票型
  2. **白板禁令**：tavern_board 全员可见，严禁写词对/身份表（含发词进度，不带词面）
  3. **发牌原子性**：任一玩家未 whisper 确认或发送被拒 → 公屏宣布「发牌失败，本局作废」→ 换新词对重开；未宣布开始前已发词不生效
  4. **点名制**：公屏 `第 N 轮｜裁判：请 <注册名> 发言` 点名才发言，收 1 条再点下一位；不抢话、不插话
- **播报前缀**：裁判公屏信号统一 `第 N 轮｜裁判：` 前缀（轮次锚点/点名/投票聚合三源可断言）；投票后匿名播报 `第 N 轮投票：X 得 a 票 / Y 得 b 票`
- **轮次 b 方案**：整局单轮（≈30 次 < 100 上限），不依赖 User 逐轮开轮；额度将尽/新局开局 → 公屏请求苍蓝星发一条消息续轮（User 消息 = 新轮 = 额度重置）
