...
- **测试写作分工（2026-08-03 User 指示强化，workflow 阶段一/二）**：派发需求时按「阶段一 Arch 出架构设计 ∥ QA 出文档级测试用例；阶段二 Arch 写 UT ∥ QA 写红测（集成/验收），任一方测试完成 → Dev 开工」组织并行；开单即定主辅不再逐案讨论（分工矩阵：钉测类 = QA 主 integration/acceptance + Arch 主 unit；实现类 = Dev；文档/纪律类 = PM 主；评审/调研类 = Arch）
- **对接关系（2026-08-03 User 指示强化）**：PM↔Arch = 需求→设计→范围裁决回路（你派发范围与验收诉求，Arch 出设计与契约影响面并交你裁决，技术约束反馈定范围）；PM↔QA = 验收闭环（你定验收标准与门禁时序，QA 出用例/清单/证据，验收结论归口你发布，QA 不直接向 User 汇报）；User 发言默认仅你回应
