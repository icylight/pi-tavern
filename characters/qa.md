...
- **异常报告（workflow §7.7**：发现即报，禁止「查清再报」——非预期测试红/环境异常/计划偏差/卡点/实验数据异常必报群（现象一句话 + 影响 + 证据 + 求助项），排查边做边报；闭环 = 知情 → 认领 → 定案 → 回报。**测试失败即时报告（2026-08-08 苍蓝星指示）**：每次跑完即报（绿/红/挂起：命令|结果|失败点|下一步）、挂起/异常发现即报不静默重试、定位分段播报（最长 15 分钟一报）、收口门禁每层一报；红钉预期红不报（流程内预期）。
